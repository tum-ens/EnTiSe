...
def generate_method_docs(app):
    """Generate documentation for all method classes."""
    import os
    import sys
    ...
    # Create absolute paths for template and output directory
    template_path = os.path.join(source_dir, "_templates", "method.rst")
    output_dir = os.path.join(source_dir, "methods")

    # Discover all method classes
    method_classes = discover_method_classes("entise.methods")

    # Generate documentation for each method
    generate_docs_for_all_methods(method_classes, template_path, output_dir)
