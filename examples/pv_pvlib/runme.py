...
Benchmark-compatible: exposes get_input() and simulate().

Core plotting logic is preserved; it is only wrapped so it doesn't execute on import.
"""
...
def get_input(path: str):
    ...
    ----------
    path : str
        Example folder path (e.g., "examples/pv_pvlib")
    ...
    """
    # Keep same logic, but make it path-safe (don't rely on cwd=".")
...
def simulate(
    ...
    - Runs TimeSeriesGenerator
    - Keeps supervisor plotting logic unchanged
    - Allows benchmarking to disable export/plot
...
if __name__ == "__main__":
    ...
    simulate(objects, data, workers=1, path=EXAMPLE_DIR, export=True, plot=True)