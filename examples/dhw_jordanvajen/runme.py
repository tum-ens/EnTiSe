...
for i, (obj_id, object) in enumerate(objects.iterrows()):
    plot_dhw_demand(axes[i], obj_id, df,
                    f'Building ID: {obj_id} - Dwelling Size: {object.dwelling_size} m²')
...
def plot_daily_profile(ax, obj_data, title):
    ...
    yearly_total = obj_data[f'load_{Types.DHW}_volume'].sum() / 1000  # Convert L to m3
    ax.text(0.95, 0.95, f'Yearly total: {yearly_total:.1f} m³',
...
for i, (obj_id, object) in enumerate(objects.iterrows()):
    plot_daily_profile(axes[i], df[obj_id][Types.DHW],
                       f'Building ID: {obj_id} - Dwelling Size: {object.dwelling_size} m²')
