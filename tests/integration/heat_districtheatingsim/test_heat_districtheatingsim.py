"""
Integration tests for EnTiSe DistrictHeatingSim heat method.

This module runs the public examples end to end through the high-level
simulation pipeline to validate that:
- The pipeline executes without errors for the DistrictHeatingSim heat example
...
def example_inputs():
    """Load example objects and data for the heat DistrictHeatingSim example from the repo.

    Ensures required inputs exist and that weather contains the C.DATETIME column.
    """
    repo = _repo_root()
    example_dir = repo / "examples" / "heat_districtheatingsim"
...
def test_heat_districtheatingsim_example_runs_end_to_end(example_inputs):
    """
    Integration test:
