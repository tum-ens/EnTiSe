...
import numpy as np
import pytest
from entise.methods.pv.pvlib import PVLib, get_input_data, calculate_timeseries
...
def dummy_inputs(dummy_weather, dummy_pv_arrays, dummy_pv_inverter):
    ...
    data = {
        O.WEATHER: dummy_weather,
        "pv_arrays": dummy_pv_arrays,
        "pv_inverter": dummy_pv_inverter
    }
...
def test_generate_method(dummy_inputs):
    ...
    assert f'{C.GENERATION}_{Types.PV}' in summary
    assert f'{O.GEN_MAX}_{Types.PV}' in summary
    assert f'{C.FLH}_{Types.PV}' in summary

    # Check that generation values are non-negative
    assert summary[f'{C.GENERATION}_{Types.PV}'].item() >= 0
    assert summary[f'{O.GEN_MAX}_{Types.PV}'].item() >= 0
    assert summary[f'{C.FLH}_{Types.PV}'].item() >= 0

    # Check timeseries values
    ts = result["timeseries"]
    assert f'{C.POWER}_{Types.PV}' in ts.columns
    assert len(ts) == len(data[O.WEATHER])
    assert (ts >= 0).all().all()
