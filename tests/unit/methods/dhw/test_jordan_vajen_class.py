...
def test_jordan_vajen_basic():
    ...
    assert f'{C.DEMAND}_{Types.DHW}_volume_total' in summary
    assert f'{C.DEMAND}_{Types.DHW}_volume_avg' in summary
    assert f'{C.DEMAND}_{Types.DHW}_volume_peak' in summary
    assert f'{C.DEMAND}_{Types.DHW}_energy_total' in summary
    assert f'{C.DEMAND}_{Types.DHW}_energy_avg' in summary
    assert f'{C.DEMAND}_{Types.DHW}_energy_peak' in summary
    assert f'{C.DEMAND}_{Types.DHW}_power_avg' in summary
    assert f'{C.DEMAND}_{Types.DHW}_power_max' in summary
    assert f'{C.DEMAND}_{Types.DHW}_power_min' in summary

    # Check that the timeseries has the expected columns
    timeseries = result['timeseries']
    assert f'{C.LOAD}_{Types.DHW}_volume' in timeseries.columns
    assert f'{C.LOAD}_{Types.DHW}_energy' in timeseries.columns
    assert f'{C.LOAD}_{Types.DHW}_power' in timeseries.columns

    # Check that the values are reasonable
    assert summary[f'{C.DEMAND}_{Types.DHW}_volume_total'] > 0
    assert summary[f'{C.DEMAND}_{Types.DHW}_energy_total'] > 0
    assert np.all(timeseries[f'{C.LOAD}_{Types.DHW}_volume'] >= 0)
    assert np.all(timeseries[f'{C.LOAD}_{Types.DHW}_energy'] >= 0)
    assert np.all(timeseries[f'{C.LOAD}_{Types.DHW}_power'] >= 0)
...
def test_jordan_vajen_reproducibility():
    ...
    pd.testing.assert_series_equal(
        result1['timeseries'][f'{C.LOAD}_{Types.DHW}_volume'],
        result2['timeseries'][f'{C.LOAD}_{Types.DHW}_volume']
    )
    pd.testing.assert_series_equal(
        result1['timeseries'][f'{C.LOAD}_{Types.DHW}_energy'],
        result2['timeseries'][f'{C.LOAD}_{Types.DHW}_energy']
    )
    ...
    with pytest.raises(AssertionError):
        pd.testing.assert_series_equal(
            result1['timeseries'][f'{C.LOAD}_{Types.DHW}_volume'],
            result3['timeseries'][f'{C.LOAD}_{Types.DHW}_volume']
        )
...
def test_jordan_vajen_dwelling_size():
    ...
    # Check that the results are different
    assert result1['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total'] < result2['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total']
    assert result1['summary'][f'{C.DEMAND}_{Types.DHW}_energy_total'] < result2['summary'][f'{C.DEMAND}_{Types.DHW}_energy_total']
...
def test_jordan_vajen_seasonal_variation():
    ...
    ts2 = result2['timeseries'][f'{C.LOAD}_{Types.DHW}_volume']
...
def test_jordan_vajen_edge_cases():
    ...
    assert result_small['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total'] > 0  # Should still produce some demand
    ...
    assert result_large['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total'] > 0  # Should produce demand
    ...
    assert np.all(result_equal_temp['timeseries'][f'{C.LOAD}_{Types.DHW}_energy'] == 0)  # Energy should be zero
...
def test_jordan_vajen_time_resolution():
    ...
    hourly_total = result_hourly['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total']
    daily_total = result_daily['summary'][f'{C.DEMAND}_{Types.DHW}_volume_total']
